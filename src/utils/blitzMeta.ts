...
function aggregate(rows: RawRow[]): BlitzAgentStat[] {
  ...
  for (const row of rows) {
    ...
    acc.m += Number(g.matchesPlayed ?? 0);
    acc.w += Number(g.wins ?? 0);
    acc.r += Number(g.roundsPlayed ?? 0);
    acc.s += Number(g.score ?? 0);
...
export async function fetchBlitzAgentStats(mapName: string, tier: number): Promise<BlitzAgentStat[]> {
  ...
  const task = (async () => {
    ...
    // rather than pretending the map has no meta.
    if (isRanked) rows = await query(true);
...
export function peekBlitzAgentStats(mapName: string, tier: number): BlitzAgentStat[] | null {
  ...
  try {
    ...
    const key = `${slug}|${tier > 0 ? tier : 'all'}|`;
    const hit = Object.entries(all).find(([k]) => k.startsWith(key));
    if (!hit) return null;
    const [k, entry] = hit;
